<script>
    ...
    let playbackRateIndex =
        availablePlaybackRates.indexOf(1) !== -1
            ? availablePlaybackRates.indexOf(1)
            : 0;
    ...
    $: {
        ...
        if (params) {
            ...
            const nextPlaylistBufferTime =
                params.get("playlistBufferTime") || "";
            ...
            if (
                nextSharedSessionId &&
                nextSharedSessionId !== sharedSessionId
            ) {
    ...
    async function waitForPlayerMountpoint(
        signal,
        { maxAttempts = 40, delayMs = 50 } = {},
    ) {
    ...
    async function initialiseBackend(videoId, reason = "unknown") {
        ...
        try {
            ...
            // Wait for player to be ready (max 10s)
            if (playerReadyPromise) {
                const readyOrAborted = await Promise.race([
                    playerReadyPromise,
                    ...
                    new Promise((resolve) => {
                        ...
                        // Also hook into signal directly if possible but interval is safe
                ...
                if (readyOrAborted === "timeout") {
                    throw new Error("YouTube player initialisation timed out");
                }
                if (readyOrAborted === "aborted" || controller.signal.aborted) {
                    return;
                }
            }
            ...
            if (!controller.signal.aborted) {
                console.error(
                    "Failed to initialise backend recorder flow",
                    error,
                    { reason },
                );
                // Reset player on failure to avoid stale state
                resetPlayer();
            }
        } finally {
    ...
    function triggerBackendInitialisation(reason = "direct") {
        ...
        if (
            hasInitialisedBackend &&
            lastInitialisedVideoId === originalVideoId
        ) {
            return;
        }
    ...
    function onPlayerReady(event) {
        if (event?.target === playerOriginal) {
            ...
            availablePlaybackRates =
                Array.isArray(playerRates) && playerRates.length
            ...
            const fallbackRate = availablePlaybackRates.includes(desiredRate)
                ? desiredRate
                : availablePlaybackRates.includes(1)
                  ? 1
                  : availablePlaybackRates[0];
            playbackRateIndex = Math.max(
                availablePlaybackRates.indexOf(fallbackRate),
                0,
            );
    ...
    function logPlaybackRateChange(newPlaybackRate) {
        if (startTime) {
            ...
            playbackRateConfigsArray = Array.from(
                playbackRateConfigs.entries(),
            );

            const playbackRateConfigsObject =
                Object.fromEntries(playbackRateConfigs);
            updateFirebaseDocument({
    ...
    function applyPlaybackRate(
        ...
        if (
            playerOriginal &&
            typeof playerOriginal.setPlaybackRate === "function"
        ) {
    ...
    function updatePlaybackRateFromIndex(
        index,
        { userInitiated = false } = {},
    ) {
        if (
            !Array.isArray(availablePlaybackRates) ||
        ...
        const clampedIndex = Math.max(
            0,
            Math.min(normalizedIndex, availablePlaybackRates.length - 1),
        );
        const selectedRate = availablePlaybackRates[clampedIndex] ?? 1;
        playbackRateIndex = clampedIndex;
        applyPlaybackRate(selectedRate, {
            shouldLog: userInitiated,
            syncSession: true,
        });
    }
    ...
    function setVolumeForOriginalVideo(volume) {
        if (playerOriginal && typeof playerOriginal.setVolume === "function") {
            ...
            console.error(
                "Player not ready or setVolume method not available.",
            );
        }
    }
    ...
    const onClickStartVideo = () => {
        ...
        if (!started) {
            console.warn(
                "YouTube player not ready yet; playback will start once it loads.",
            );
        }
    };
    ...
    const onClickFinishReaction = async () => {
        ...
        if (sharedSessionId) {
            try {
                ...
                console.error(
                    "Failed to update shared session at finish:",
                    error,
                );
            }
        }
        ...
        if (nextVideoId) {
            const nextRoute =
                ...
                (sharedSessionId ? `&sharedSessionId=${sharedSessionId}` : "");
            ...
            <main class="grid flex-1 gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <aside class="flex w-full flex-col gap-4">
                    ...
                    >
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-xs text-slate-500">
                                {formatPlaybackRate(
                                    availablePlaybackRates[0] ?? 1,
                                )}x – {formatPlaybackRate(
                                    availablePlaybackRates[
                                        availablePlaybackRates.length - 1
                                    ] ?? 1,
                                )}x
                            </span>
                        </div>
                        ...
                        <div
                            class="mt-3 flex flex-wrap gap-2 text-[11px] text-slate-500"
                        >
                            {#each availablePlaybackRates as rate, idx}
                                <span
                                    class={`rounded-full px-2 py-1 ${idx === playbackRateIndex ? "bg-slate-800 text-slate-200" : ""}`}
                                >
                    ...
                    {#if showRecorder}
                        ...
                        >
                            <p
                                class="mb-3 text-xs uppercase tracking-[0.3em] text-slate-500"
                            >
                                Recorder
                            </p>
                    ...
                    {#if debugMode && currentButtonGroupState !== BUTTON_GROUP_STATES.INITIAL}
                        <div
                            class="rounded-3xl border border-slate-900/60 bg-slate-900/40 p-5 text-xs text-slate-300"
                        >
                            <div class="flex items-center justify-between">
                                <p
                                    class="text-xs uppercase tracking-[0.3em] text-slate-500"
                                ...
                                <span
                                    class="rounded-full bg-slate-950/60 px-2 py-1 font-mono text-[10px] text-slate-400"
                                    >DEV</span
                                >
                            </div>
                            <ul class="mt-3 space-y-1">
                                ...
                                <li>
                                    Playlist Buffer: {playlistBufferTime || 0}s
                                </li>
                                <li>
                                    Reaction Configs: {reactionConfigsArray.length}
                                </li>
                                <li>
                                ...
                                <li>
                                    Current State: {currentButtonGroupState}
                                </li>
                            </ul>
    ...
    <Modal bind:open={showShareModal} title="Share Your Reaction Session">
        <div class="space-y-4 text-slate-200">
            <p class="text-sm text-slate-400">
                Share this link so collaborators can watch your reaction live.
                Playback stays in sync with your controls.
            </p>
